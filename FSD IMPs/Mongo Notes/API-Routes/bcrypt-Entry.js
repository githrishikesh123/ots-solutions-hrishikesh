...
export const verifyEmail = async (req, res) => {
  const { email, otp } = req.body; // get email and OTP from  body
  const searchedRecord = await CustomersModel.find({ email: email });

  console.log("TEST -> ", searchedRecord);

  if (searchedRecord.length > 0) {
    if (searchedRecord[0].otp == otp) {     // if matched the OTP
      try {
        const result = await CustomersModel.updateOne(   // Update Email status by email as unique Id
          { email: email },
          {
            $set: {
              isEmailVerfified: true,      // will verify email status if OPT matched
            },
          }
        );
        if (result.acknowledged) {          // will get acknowledged" : true after update
          res.status(200).json({
            status: true,
